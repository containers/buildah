...
read
: "[1m Copy a subdirectory with some files [0m"
read
...
read
: "[1m Copy the tarball to the container [0m"
read
echo '[mkdir -p $mountpoint1/tmpwatch]'
echo '[buildah config --name $container1 --workingdir /tmpwatch]'
echo '[buildah copy --name $container1 tmpwatch-2.9.17.tar.bz2]'
read
mkdir -p $mountpoint1/tmpwatch
buildah config --name $container1 --workingdir /tmpwatch
buildah copy --name $container1 tmpwatch-2.9.17.tar.bz2
read
: "[1m Copy another tarball to the container [0m"
read
echo '[buildah copy --name $container1 https://releases.pagure.org/newt/newt-0.52.19.tar.gz]'
read
buildah copy --name $container1 https://releases.pagure.org/newt/newt-0.52.19.tar.gz
read
: "[1m List some of the container's contents [0m"
read
...
read
: "[1m Add the tarball to the container [0m"
read
echo '[buildah add --name $container1 tmpwatch-2.9.17.tar.bz2]'
read
buildah add --name $container1 tmpwatch-2.9.17.tar.bz2
read
: "[1m List some of the container's contents [0m"
read
echo '[find $mountpoint1/tmpwatch]'
