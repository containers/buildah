...
func GetCacheMount(args []string, additionalMountPoints map[string]internal.StageMountDetails, tmpDir string) (specs.Mount, string, *lockfile.LockFile, error) {
	var err error
	var mode uint64
...
func GetVolumes(ctx *types.SystemContext, store storage.Store, mountLabel string, volumes []string, mounts []string, contextDir string, tmpDir string) ([]specs.Mount, []string, []string, []string, []*lockfile.LockFile, error) {
...
func getMounts(ctx *types.SystemContext, store storage.Store, mountLabel string, mounts []string, contextDir, tmpDir string) (map[string]specs.Mount, []string, []string, []string, []*lockfile.LockFile, error) {
	...
	lockedTargets := make([]*lockfile.LockFile, 0)
	succeeded := false
	defer func() {
		if !succeeded {
