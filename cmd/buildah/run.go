...
func runCmd(c *cobra.Command, args []string, iopts runInputOptions) error {
	...
	mounts, mountedImages, intermediateMounts, _, targetLocks, err := internalParse.GetVolumes(systemContext, store, builder.MountLabel, iopts.volumes, iopts.mounts, iopts.contextDir, iopts.workingDir, tmpDir)
	if err != nil {
		return err
	}
	defer func() {
