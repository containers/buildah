...
func (ic *imageCopier) compareImageDestinationManifestEqual(ctx context.Context, targetInstance *digest.Digest) (*copySingleImageResult, error) {
	...
	if err != nil {
		logrus.Debugf("Unable to create destination image %s source: %v", ic.c.dest.Reference().StringWithinTransport(), err)
		return nil, nil
	}
	...
	if err != nil {
		logrus.Debugf("Unable to get destination image %s/%s manifest: %v", destImageSource.Reference().StringWithinTransport(), targetInstance, err)
		return nil, nil
	}
