...
@test "copy-link-flag" {
  ...
  run_buildah unmount $cid
  run_buildah commit $WITH_POLICY_JSON $cid containers-storage:copy-link-image

  run_buildah inspect --type=image copy-link-image
  layers=$(echo "$output" | jq -r '.OCIv1.rootfs.diff_ids | length')
  assert "$layers" -ge 2 "Expected at least 2 layers from 2 --link operations, but found $layers"
...
@test "copy-link-directory" {
  ...
  run_buildah commit $WITH_POLICY_JSON $cid containers-storage:copy-link-dir-image
...
@test "copy-link-with-chown" {
  ...
  run_buildah commit $WITH_POLICY_JSON $cid containers-storage:copy-link-chown-image
...
@test "copy-link-with-chmod" {
  ...
  run_buildah commit $WITH_POLICY_JSON $cid containers-storage:copy-link-chmod-image
