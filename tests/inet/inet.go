...
func main() {
	...
	// Retrieve the address we ended up bound to.
	addr := ln.Addr()
	...
	if portFile != "" {
		if err := os.WriteFile(portFile, []byte(portString), 0o644); err != nil {
			logrus.Fatalf("writing listening address to %q: %v", portFile, err)
		}
	...
	if pidFile != "" {
		...
		if err := os.WriteFile(pidFile, []byte(pid), 0o644); err != nil {
			logrus.Fatalf("writing pid %d to %q: %v", os.Getpid(), portFile, err)
		}
