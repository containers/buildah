...
@test "bud and test inherit-labels" {
  ...
  if [[ $(cat ${TEST_SCRATCH_DIR}/id1) != $(cat ${TEST_SCRATCH_DIR}/id2) ]]; then
    echo "Expected image id to not change" >&2
    false
  fi
  ...
  run_buildah build $WITH_POLICY_JSON --layers --inherit-labels=true -t exp -f $BUDFILES/base-with-labels/Containerfile.multi-stage
  want_output='map["io.buildah.version":"'$buildah_version'" "name":"world"]'
  run_buildah inspect --format '{{printf "%q" .Docker.Config.Labels}}' exp
  expect_output "$want_output"
}
