...
@test "bud with preprocessor disabled via explicit cli flag, defaulted to true via env var" {
  _prefetch busybox
  target=alpine-image
  BUILDAH_PREPROCESS=1 run_buildah 125 build $WITH_POLICY_JSON -t ${target} --preprocess=false -f Decomposed.in $BUDFILES/preprocess
  expect_output --substring 'Build error: Unknown instruction: "RUNHELLO"'
}
