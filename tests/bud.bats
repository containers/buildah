...
@test "[test labeling intermediate images] --jobs > 2 correctly resolves stage references to imageID in labels" {
  _prefetch alpine
  target_jobs1="imageid-resolution-jobs1-$(safename)"
  target_jobs2="imageid-resolution-jobs2-$(safename)"

  # Build with --jobs 1
  run_buildah build $WITH_POLICY_JSON --jobs 1 --save-stages --stage-labels --layers -t ${target_jobs1} -f $BUDFILES/save-stages/Dockerfile.chained-two-build-stages $BUDFILES/save-stages

  run_buildah images -a
  all_intermediate_images_jobs1=$(echo "$output" | awk '$1 == "<none>" && $2 == "<none>" {print $3}')

  # Find parent-stage and child-stage intermediate images
  parent_image_jobs1=""
  child_image_jobs1=""
  for image_id in $all_intermediate_images_jobs1; do
    run_buildah inspect --format '{{index .OCIv1.Config.Labels "io.buildah.stage.name"}}' $image_id
    if [ "$output" = "parent-stage" ]; then
      parent_image_jobs1="$image_id"
    elif [ "$output" = "child-stage" ]; then
      child_image_jobs1="$image_id"
    fi
  done

  assert "$parent_image_jobs1" "!=" "" "parent-stage intermediate image should exist"
  assert "$child_image_jobs1" "!=" "" "child-stage intermediate image should exist"

  # Get child-stage base label value
  run_buildah inspect --format '{{index .OCIv1.Config.Labels "io.buildah.stage.base"}}' $child_image_jobs1
  child_base_jobs1="$output"

  # Verify base label is an imageID from intermediate images (not alias "parent-stage")
  found_in_images=false
  for iid in $all_intermediate_images_jobs1; do
    if [[ "$child_base_jobs1" == "$iid"* ]]; then
      found_in_images=true
      break
    fi
  done
  assert "$found_in_images" == "true" "child stage.base should be imageID from intermediate images"

  # Clean up
  run_buildah rmi -a -f

  # Build with --jobs 2
  run_buildah build $WITH_POLICY_JSON --jobs 2 --save-stages --stage-labels --layers -t ${target_jobs2} -f $BUDFILES/save-stages/Dockerfile.chained-two-build-stages $BUDFILES/save-stages

  run_buildah images -a
  all_intermediate_images_jobs2=$(echo "$output" | awk '$1 == "<none>" && $2 == "<none>" {print $3}')

  # Find parent-stage and child-stage intermediate images
  parent_image_jobs2=""
  child_image_jobs2=""
  for image_id in $all_intermediate_images_jobs2; do
    run_buildah inspect --format '{{index .OCIv1.Config.Labels "io.buildah.stage.name"}}' $image_id
    if [ "$output" = "parent-stage" ]; then
      parent_image_jobs2="$image_id"
    elif [ "$output" = "child-stage" ]; then
      child_image_jobs2="$image_id"
    fi
  done

  assert "$parent_image_jobs2" "!=" "" "parent-stage intermediate image should exist"
  assert "$child_image_jobs2" "!=" "" "child-stage intermediate image should exist"

  # Get child-stage base label value
  run_buildah inspect --format '{{index .OCIv1.Config.Labels "io.buildah.stage.base"}}' $child_image_jobs2
  child_base_jobs2="$output"

  # Verify base label is an imageID from intermediate images (not alias "parent-stage")
  found_in_images=false
  for iid in $all_intermediate_images_jobs2; do
    if [[ "$child_base_jobs2" == "$iid"* ]]; then
      found_in_images=true
      break
    fi
  done
