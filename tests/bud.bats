...
FROM $SAFEIMAGE
_EOF
  run_buildah build --pull=false -q --arch=amd64 -t image-amd --signature-policy ${TESTSDIR}/policy.json ${mytmpdir}
  ...
  run_buildah build --pull=false -q --arch=arm64 -t image-arm --signature-policy ${TESTSDIR}/policy.json ${mytmpdir}
...
_EOF
  ...
  run_buildah build $WITH_POLICY_JSON -f ${TEST_SCRATCH_DIR}/Containerfile
