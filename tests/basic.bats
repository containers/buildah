...
@test "by-name" {
	cid=$(buildah from --pull --signature-policy ${TESTSDIR}/policy.json alpine alpine-working-image-for-test)
...
@test "commit" {
	...
	run buildah commit $newcid  --signature-policy ${TESTSDIR}/policy.json ontainers-storage:rejected-new-image 
	[ "$status" -eq 1 ]
