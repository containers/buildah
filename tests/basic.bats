...
@test "from" {
  ...
  run_buildah rm $cid
  cid=$(buildah from --pull "missing" --signature-policy ${TESTSDIR}/policy.json --name i-love-naming-things alpine)
...
@test "by-name" {
  cid=$(buildah from --pull "missing" --signature-policy ${TESTSDIR}/policy.json --name scratch-working-image-for-test scratch)
