...
func (s *StageExecutor) performCopy(excludes []string, copies ...imagebuilder.Copy) error {
	...
	for _, copy := range copies {
		...
		options := buildah.AddAndCopyOptions{
			...
			Link:                  s.hasLink,
			Annotation:            s.Annotations(s.isLastStep),
		}
		if len(copy.Files) > 0 {
...
func (s *StageExecutor) getCreatedBy(node *parser.Node, addedContentSummary string, isLastStep bool) (string, error) {
	...
	case "ADD", "COPY":
		...
		return "/bin/sh -c #(nop) " + strings.ToUpper(node.Value) + link + " " + addedContentSummary + " in " + destination.Value + " " + inheritLabels + " " + unsetAnnotations + " " + inheritAnnotations + " " + newAnnotations, nil
	default:
...
func (s *StageExecutor) Annotations(isLastStep bool) string {
	if !isLastStep {
		return "   "
	}
