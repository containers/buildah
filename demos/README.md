...
filename: [`buildah-scratch-demo.sh`](https://github.com/projectatomic/buildah/blob/master/demos/buildah-scratch-demo.sh)
...
filename: [`docker-compatibility-demo.sh`](https://github.com/projectatomic/buildah/blob/masterdemos/docker-compatibility-demo.sh)
...
## Buildah, Build Using Dockerfile (bud) demo 

filename: [`buildah-bud-demo.sh`](https://github.com/projectatomic/buildah/blob/masterdemos/buildah-bud-demo.sh)

This demo builds an nginx container image using Buildah's `build-using-docker`, or `bud`, option . It builds the same image as the Docker compatibility demo (at time of writing). The container is tested using `podman run` and then stopped. The Docker daemon is then started and the image is pushed to the Docker repository. The container is started using `docker run` and tested. 
